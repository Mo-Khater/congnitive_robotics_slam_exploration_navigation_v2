...
class DWAPlanner:
    """Improved DWA: Better path tracking with stuck detection and recovery"""
    def __init__(self):
        ...
        # Path following - MORE FLEXIBLE: Smooth curves, good speed
        self.lookahead_min = 0.3   # Longer lookahead
        self.lookahead_max = 0.8   # Much more flexibility
        self.goal_tolerance = rospy.get_param('~goal_tolerance', 0.35)
        
        # Cost weights - PRIORITIZE SPEED & SMOOTHNESS
        self.goal_cost_weight = 1.2      # Lower - allow deviation (was 2.0)
        self.speed_cost_weight = 0.6     # Higher - want speed! (was 0.5)
        self.obstacle_cost_weight = 1.2  # Lower - be braver
        ...
        # Stuck detection
        self.consecutive_zero_velocity = 0
        self.stuck_threshold = 10  # 1 second of zero velocity (10 * 0.1s)
        ...
        # Subscribers
        self.path_sub = rospy.Subscriber('/robot3/global_path', Path, self.path_callback)
        self.odom_sub = rospy.Subscriber('/robot3/odom', Odometry, self.odom_callback)
        self.scan_sub = rospy.Subscriber('/robot3/scan', LaserScan, self.scan_callback)
        
        # Publishers
        self.cmd_pub = rospy.Publisher('/robot3/cmd_vel', Twist, queue_size=10)
        self.local_path_pub = rospy.Publisher('/robot3/local_path', Path, queue_size=10)
        self.goal_reached_pub = rospy.Publisher('/robot3/goal_reached', Bool, queue_size=10)
        self.goal_failed_pub = rospy.Publisher('/robot3/goal_failed', Bool, queue_size=10)
        
        # Control loop
        self.control_timer = rospy.Timer(rospy.Duration(0.1), self.control_loop)
        
        rospy.loginfo("Flexible Local Planner initialized - Smooth, fast navigation with stuck detection")
    ...
    def calc_obstacle_cost(self, trajectory):
        """Obstacle cost - balanced with better recovery"""
        ...
        # Safety margin - slightly more aggressive
        safety_margin = self.robot_radius * 0.85
        if min_dist < safety_margin:
            return float('inf')
        
        # Soft cost - more gradual
        danger_zone = 0.5
        if min_dist < danger_zone:
            cost = (danger_zone - min_dist) / (danger_zone - safety_margin)
            return cost * 0.4  # Even softer penalty
    ...
    def get_closest_path_point(self):
        ...
        try:
            ...
            for pose in self.global_path.poses:
                ...
                try:
                    p_base = self.tf_buffer.transform(p_current, 'robot3_base_link', 
                                                     rospy.Duration(0.1))
                except:
                    continue
    ...
    def calc_goal_cost(self, trajectory):
        """Goal cost - FLEXIBLE: General direction more important than exact path"""
        ...
        # FLEXIBLE: Mostly care about general direction
        cost = angle_diff * 0.5 + final_dist * 0.6
        
        # Very gentle penalty for lateral deviation - allow smooth shortcuts
        if target_x > 0.01:
            cross_track = abs(final_y * np.cos(target_angle) - final_x * np.sin(target_angle))
            cost += cross_track * 0.3  # Very gentle - freedom to cut corners
        
        return cost
        
    def calc_speed_cost(self, v):
        """Speed cost - strongly prefer speed"""
        if v < 0:
            return 2.5
        
        # Prefer 75% of max speed - fast!
        target_speed = self.max_vel_x * 0.75
        speed_diff = abs(v - target_speed) / self.max_vel_x
        
        # Very small penalty - prioritize speed
    ...
    def dwa_control(self):
        ...
        # Recovery if blocked
        if valid_count == 0:
            rospy.logwarn_throttle(1.0, "⚠️ BLOCKED - No valid trajectories! Starting recovery rotation")
            # Try rotating in place to find a way around
            best_v = 0.0
            best_w = 0.7 if self.current_vel[1] >= 0 else -0.7  # Rotate consistently
            best_trajectory = self.predict_trajectory(best_v, best_w)
        elif valid_count < 3:
            rospy.logwarn_throttle(2.0, f"⚠️ Limited options ({valid_count} trajectories)")
            # Very limited options - prefer rotation over stopping
            if abs(best_v) < 0.05 and abs(best_w) < 0.1:
                rospy.logwarn_throttle(1.0, "Forcing recovery rotation")
                best_v = 0.0
                best_w = 0.5
    ...
    def distance_to_goal(self):
        ...
        try:
            ...
            goal_base = self.tf_buffer.transform(goal_current, 'robot3_base_link',
                                                rospy.Duration(0.1))
    ...
    def check_making_progress(self):
        ...
        # Calculate movement in 3 seconds
        movement = np.linalg.norm(current_pos - self.position_at_last_check)
        
        self.position_at_last_check = current_pos
        self.last_progress_check = now
        
        # Need at least 25cm in 3 seconds (more lenient during rotation)
        min_required = 0.25
        if abs(self.current_vel[1]) > 0.3:  # If rotating
            min_required = 0.15  # Allow less linear movement during rotation
        
        if movement < min_required:
            rospy.logwarn(f"⚠️ Poor progress: moved only {movement:.2f}m in 3 seconds")
            return False
    ...
    def control_loop(self, event):
        ...
        if not self.check_making_progress():
            ...
            # Give 4 chances (12 seconds total) - allows time for rotation recovery
        ...
        # Detect if stuck (zero velocity for too long)
        if abs(v) < 0.05 and abs(w) < 0.05:
            self.consecutive_zero_velocity += 1
            
            if self.consecutive_zero_velocity >= self.stuck_threshold:
                rospy.logwarn(f"🚨 STUCK! Zero velocity for {self.consecutive_zero_velocity} cycles - forcing rotation")
                # Force aggressive rotation to escape
                v = 0.0
                w = 0.8 if not hasattr(self, 'last_recovery_direction') else -self.last_recovery_direction
                self.last_recovery_direction = w
                trajectory = self.predict_trajectory(v, w)
        else:
            self.consecutive_zero_velocity = 0
        ...
        rospy.loginfo_throttle(1.0, 
            f"robot3 v={v:.2f} w={w:.2f} | dist={dist_to_goal:.2f}m | t={pursuit_time:.0f}s")
    ...
    def publish_local_path(self, trajectory):
        ...
        path_msg.header.frame_id = "robot3_base_link"
