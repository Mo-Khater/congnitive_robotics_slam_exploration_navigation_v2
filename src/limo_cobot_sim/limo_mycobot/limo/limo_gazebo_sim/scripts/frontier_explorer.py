...
class FrontierExplorer:
    ...
    def __init__(self):
        ...
        # Subscribers
        self.map_sub = rospy.Subscriber('/robot1/map', OccupancyGrid, self.map_callback)
        self.goal_reached_sub = rospy.Subscriber('/robot1/goal_reached', Bool, self.goal_reached_callback)
        self.goal_failed_sub = rospy.Subscriber('/robot1/goal_failed', Bool, self.goal_failed_callback)
        
        # Publishers
        self.goal_pub = rospy.Publisher('/robot1/goal', PoseStamped, queue_size=10)
        self.frontier_pub = rospy.Publisher('/robot1/frontiers', MarkerArray, queue_size=10)
    ...
    def goal_failed_callback(self, msg):
        ...
        if msg.data:
            rospy.logwarn("❌ robot1 Goal failed - will avoid this area")
            self.goal_active = False
    ...
    def publish_frontiers_markers(self, frontiers, robot_pose):
        ...
        for i, frontier in enumerate(frontiers):
            ...
            marker = Marker()
            marker.header.frame_id = "robot1_map"
    ...
    def exploration_callback(self, event):
        ...
        if len(blocked_by_failed) > 0 and len(available_after_failed_filter) <= 3:
            rospy.logwarn("="*70)
            rospy.logwarn(f"📍robot1 Only {len(available_after_failed_filter)} available frontiers, "
