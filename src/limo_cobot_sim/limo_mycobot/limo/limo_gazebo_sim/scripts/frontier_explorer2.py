...
class FrontierExplorer:
    ...
    def __init__(self):
        ...
        self.goal_reached_sub = rospy.Subscriber('/robot2/goal_reached', Bool, self.goal_reached_callback)
        self.goal_failed_sub = rospy.Subscriber('/robot2/goal_failed', Bool, self.goal_failed_callback)
        
        # Publishers
        self.goal_pub = rospy.Publisher('/robot2/goal', PoseStamped, queue_size=10)
        self.frontier_pub = rospy.Publisher('/robot2/frontiers', MarkerArray, queue_size=10)
    ...
    def goal_failed_callback(self, msg):
        ...
        if msg.data:
            rospy.logwarn("❌ robot2 Goal failed - will avoid this area")
            self.goal_active = False
    ...
    def get_robot_pose(self):
        ...
        try:
            trans = self.tf_buffer.lookup_transform('robot2_map', 'robot2_base_link', 
                                                   rospy.Time(0), rospy.Duration(1.0))
            pose = PoseStamped()
            pose.header.frame_id = 'robot2_map'
    ...
    def publish_frontiers_markers(self, frontiers, robot_pose):
        ...
        for i, frontier in enumerate(frontiers):
            ...
            marker = Marker()
            marker.header.frame_id = "robot2_map"
    ...
    def exploration_callback(self, event):
        ...
        if len(blocked_by_failed) > 0 and len(available_after_failed_filter) <= 3:
            rospy.logwarn("="*70)
            rospy.logwarn(f"📍robot2 Only {len(available_after_failed_filter)} available frontiers, "
        ...
        goal = PoseStamped()
        goal.header.frame_id = "robot2_map"
